<template>
	<v-container>
		...
		<v-dialog v-model="dialog.sessionCode" persistent :close-on-back="false" max-height="80%" max-width="80%" width="200px">
			<v-card>
				<v-card-text class="text-center">
				...
				<v-card-actions>
					<v-btn color="red" block @click="dialog.sessionCode = false">Cancel</v-btn>
					<v-spacer />
					<v-btn color="red" block @click="join">Join</v-btn>
				</v-card-actions>
			</v-card>
		</v-dialog>
	</v-container>
	...
	methods: {
		...
		async join() {
			if (this.info.name === null || this.info.name === '') {
				...
				this.dialog.info = false
				this.$router.push({ name: 'initiative-session-id', params: { id: 'CANTBT' } })
			}
		},
		saveInfo(info) {
			this.dialog.info = false
